...
""" Generic Option Parser Module.

This module provides the genericOptionparser which is used to write code
that will parse command line options for an application by invoking one of
the standard Python library command argument parser modules optparse or getopt.

The class first tries to use optparse. It it is not there (< Python 2.3),
it invokes getopt. However, this istransparent to the application which uses
the class.

The class requires a dictionary with entries of the following form for each
command line option.
...
where, 'option_var' is the key for the option in the final dictionary of
option-value pairs. The value is a tuple of strings, where each string consists
of entries of the form,

'key=value', where 'key' is borrowed from the way optparse represents each
variables for an option setting.

To parse the arguments, call the method 'parse_arguments'.
The return value is a dictionary of the option-value pairs.

History:
 - version 0.1.1 : adapted by Nicolas Pinto for the rpmt-py tool
 - version 0.1   : working version by Anand Pillai
"""

import sys

__author__ =" Anand Pillai "
__author__+=" Nicolas Pinto <nicolas.pinto@cern.ch>"
__version__="0.1.1"

class genericOptionParserError(Exception):
    """ Simply handles the genericOptionParser exceptions
    """
...
class genericOptionParser:
    ...
    def __init__(self, optmap, optord):
        """ genericOptionParser default constructor
        @param optmap: dictionary with entries for each command line option.
        @param optord: order of the dictionnary keys
        """
    ...
    def _usage(self):
        ...
        for value in self._optord:
            # sopt = value.get('short','')
            sopt = self._optmap[value].get('short','')
            # lopt = value.get('long','')
            lopt = self._optmap[value].get('long','')
            # help = value.get('help','')
            help = self._optmap[value].get('help','')
            # meta = value.get('meta','')
            meta = self._optmap[value].get('meta','')            
            
            optstr = ""
            if sopt: optstr="".join((optstr,'  -',sopt,meta))
            if lopt:
                if sopt:
                    optstr += ','
                else:
                    optstr += ' '
                optstr="".join((optstr,' --',lopt))                    
...
if __name__=="__main__":
    d = {
        'infile' : ('long=in','help=Input file for the program',
                    'meta=IN'),
